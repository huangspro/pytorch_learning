...
class MyDataset(torch.utils.data.Dataset):
    def __init__(self):
        X = pathlib.Path("../pet_data_small/")
        Y = pathlib.Path("../pet_data_mask_small/")
    ...
    def __getitem__(self, idx):
        return (transform(PIL.Image.open(self.X_data[idx]).convert('RGB')), transform(PIL.Image.open(self.X_data[idx]).convert('RGB')))
        
my_dataset = MyDataset()
print("data loaded" + "-"*50)

loader = torch.utils.data.DataLoader(my_dataset, shuffle = True, batch_size = 29)
...
class net(torch.nn.Module):
    def __init__(self):
        ...
        self.bn6 = torch.nn.BatchNorm2d(32)
        self.transpose_conv4 = torch.nn.ConvTranspose2d(32, 3, kernel_size=4, stride=2, padding=1)
...
for i in range(0, 200):
    ...
    for id,(x,y) in enumerate(loader):
        ...
        print((id + 1)*29/len(my_dataset))
        x_data.append(iid)
        y_data.append(total_loss/29/(id + 1))
        ax.clear()
...
image2 = model(my_dataset[1][0].unsqueeze(0))[0].permute(1,2,0)
image2 = image2.detach()

image2 = image2.reshape(65536, 3)

newimage = torch.zeros(65536)
i = 0

while i < 256*256:
    newimage[i] = torch.argmax(image2[i])
    i += 1
image2 = newimage
